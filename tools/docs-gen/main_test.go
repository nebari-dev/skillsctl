	...
	for _, name := range wantFiles {
		t.Run(name, func(t *testing.T) {
			...
			if !strings.HasPrefix(content, "---\ntitle: ") {
				t.Errorf("%s: missing frontmatter; first 40 bytes: %q", name, content[:min(len(content), 40)])
			}
			...
			if strings.Contains(content, "relref") {
				t.Errorf("%s: cross-links must be plain paths, not Hugo relref shortcodes", name)
			}
		})
	}
}
