...
Set `oidc.issuerURL` and `oidc.clientID` to enable authentication. See [Configuration reference](/server/configuration/) for details on OIDC setup.
...
If you are running on a Nebari cluster, use the [NebariApp integration](/server/nebari/) instead of enabling Ingress.
...
See [Nebari integration](/server/nebari/) for details.
...
- [Nebari integration](/server/nebari/) - NebariApp CRD for Nebari clusters
- [Configuration reference](/server/configuration/) - all environment variables and OIDC setup
