...
When deploying with Helm, set OIDC values through the chart's `oidc` values block. The chart maps these to the corresponding environment variables in the pod spec. See [Kubernetes deployment](/server/kubernetes/) for the full values reference.
